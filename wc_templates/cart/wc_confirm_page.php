...
<div id="tile-container" class="two-column container">
<div class="catbox">
...
<?php if( usces_is_member_system() && usces_is_member_system_point() && usces_is_login() && usces_is_available_point() ) : ?>
		...
		<div class="error_message"><?php usces_error_message(); ?></div>
		<table cellspacing="0" id="point_table" class="point-table">
			<tr>
				<td><?php _e('The current point', 'usces'); ?></td>
				<td><span class="point m1"><?php echo $usces_members['point']; ?></span>pt</td>
			</tr>
			<tr>
				<td><?php _e('Points you are using here', 'usces'); ?></td>
				<td><input name="offer[usedpoint]" class="used_point m1" type="text" value="<?php echo esc_attr($usces_entries['order']['usedpoint']); ?>" />pt</td>
			</tr>
				<tr>
				<td colspan="2"><input name="use_point" type="submit" class="use_point_button m1" value="<?php _e('Use the points', 'usces'); ?>" /></td>
			</tr>
	</table>
